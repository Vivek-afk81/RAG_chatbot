import os
import gradio as gr
from dotenv import load_dotenv
from huggingface_hub import InferenceClient

from rag.loader import load_pdf
from rag.chunker import chunk_text, compare_chunk_sizes, CHUNK_PRESETS
from rag.embedder import build_index, query_index
from rag.search import semantic_search, answer_question


# Setup


load_dotenv()
HF_TOKEN = os.getenv("HF_TOKEN")
client = InferenceClient(api_key=HF_TOKEN)

# Global state 
state = {
    "chunks": [],
    "index": None,
    "chunk_preset": "medium",
    "pdf_name": None,
    "raw_text": "",
}

CHUNK_SIZE_MAP = {"Small": "small", "Medium": "medium", "Large": "large"}


# Core logic helpers


def process_pdf(pdf_file, chunk_size_label):
    """Load PDF, chunk it, build FAISS index. Returns stat strings."""
    if pdf_file is None:
        return "—", "—", "—", "—", "No file uploaded.", []

    preset_key = CHUNK_SIZE_MAP.get(chunk_size_label, "medium")
    preset = CHUNK_PRESETS[preset_key]

    # In Gradio 6, gr.File with type="filepath" returns the path string directly
    path = pdf_file if isinstance(pdf_file, str) else pdf_file.name
    text = load_pdf(path)
    chunks = chunk_text(text, **preset)
    index = build_index(chunks)

    state["chunks"] = chunks
    state["index"] = index
    state["chunk_preset"] = preset_key
    state["pdf_name"] = os.path.basename(path)
    state["raw_text"] = text

    avg_chars = int(sum(len(c) for c in chunks) / len(chunks)) if chunks else 0

    comparison = compare_chunk_sizes(text)
    comparison_rows = [
        [label, s["chunk_size"], s["total_chunks"], s["avg_chars"]]
        for label, s in comparison.items()
    ]

    return (
        str(len(chunks)),
        "384",
        str(preset["chunk_size"]),
        str(avg_chars),
        f"Index ready — {index.ntotal} vectors ({preset_key} chunks)",
        comparison_rows,
    )


def rebuild_index(chunk_size_label):
    """Re-chunk + re-index when user changes chunk size after upload."""
    if state["index"] is None or not state["raw_text"]:
        return "—", "—", "—", "—", "Upload a PDF first.", []

    preset_key = CHUNK_SIZE_MAP.get(chunk_size_label, "medium")
    preset = CHUNK_PRESETS[preset_key]

    text = state["raw_text"]
    chunks = chunk_text(text, **preset)
    index = build_index(chunks)

    state["chunks"] = chunks
    state["index"] = index
    state["chunk_preset"] = preset_key

    avg_chars = int(sum(len(c) for c in chunks) / len(chunks)) if chunks else 0

    comparison = compare_chunk_sizes(text)
    comparison_rows = [
        [label, s["chunk_size"], s["total_chunks"], s["avg_chars"]]
        for label, s in comparison.items()
    ]

    return (
        str(len(chunks)),
        "384",
        str(preset["chunk_size"]),
        str(avg_chars),
        f"✓ Rebuilt — {index.ntotal} vectors ({preset_key} chunks)",
        comparison_rows,
    )


def chat_respond(user_message, history, mode):
    """
    Handle one turn of conversation.
    Gradio 6: history is a list of {"role": ..., "content": ...} dicts.
    """
    if not user_message.strip():
        return history, ""

    if state["index"] is None:
        history = history + [
            {"role": "user", "content": user_message},
            {"role": "assistant", "content": "Upload and process a PDF first."},
        ]
        return history, ""

    if mode == "Search":
        results = semantic_search(
            user_message, state["index"], state["chunks"], k=3
        )
        lines = [f"**Semantic search results for:** *{user_message}*\n"]
        for r in results:
            lines.append(
                f"**Rank {r['rank']} · score {r['score']:.3f}**\n"
                f"> {r['passage'][:400].strip()}…\n"
            )
        response = "\n".join(lines)
    else:
        try:
            raw_answer = answer_question(
                user_message, state["index"], state["chunks"], client
            )
            retrieved_chunks, scores = query_index(
                user_message, state["index"], state["chunks"], k=3
            )
            chunk_labels = []
            for chunk_text_val in retrieved_chunks:
                try:
                    idx = state["chunks"].index(chunk_text_val)
                    chunk_labels.append(f"`chunk {idx}`")
                except ValueError:
                    chunk_labels.append("`chunk ?`")
            pill_line = "  ".join(chunk_labels)
            response = f"{raw_answer}\n\n**Sources:** {pill_line}"
        except Exception as e:
            response = f"Error: {e}"

    # Gradio 6 messages format — dicts with role/content keys
    history = history + [
        {"role": "user", "content": user_message},
        {"role": "assistant", "content": response},
    ]
    return history, ""


def clear_chat():
    return [], ""



# UI

with gr.Blocks(title="RAG Document Q&A") as demo:

    
    # Header
    with gr.Row(elem_id="header"):
        gr.Markdown(
            "## RAG Document Q&A\n"
            "Upload a PDF, pick a chunk size, then ask questions or run semantic search."
        )

   
    # Main layout: sidebar 

    with gr.Row(equal_height=False):

        # ---- LEFT SIDEBAR ---- compact, no tall cards
        with gr.Column(scale=1, min_width=240, elem_id="sidebar"):

            pdf_input = gr.File(
                label="UPLOAD PDF",
                file_types=[".pdf"],
                type="filepath",
                height=100,         # compact upload box
            )

            status_box = gr.Textbox(
                label="Status",
                value="⬆ Upload a PDF to start",
                interactive=False,
                max_lines=1,
                elem_id="status",
            )

            with gr.Accordion("Chunk size & stats", open=False):
                chunk_radio = gr.Radio(
                    choices=["Small", "Medium", "Large"],
                    value="Medium",
                    label="Chunk size",
                    info="Small=300 · Medium=1000 · Large=2000 chars",
                )
                rebuild_btn = gr.Button("Rebuild index", size="sm", variant="secondary")

                # 4 stats in one row, compact
                with gr.Row():
                    stat_chunks = gr.Textbox(label="Chunks",     elem_classes="stat-box",
                                             interactive=False, max_lines=1, value="—")
                    stat_dims   = gr.Textbox(label="Dims",       elem_classes="stat-box",
                                             interactive=False, max_lines=1, value="—")
                    stat_csize  = gr.Textbox(label="Chunk size", elem_classes="stat-box",
                                             interactive=False, max_lines=1, value="—")
                    stat_avg    = gr.Textbox(label="Avg chars",  elem_classes="stat-box",
                                             interactive=False, max_lines=1, value="—")

                comparison_table = gr.Dataframe(
                    headers=["Preset", "Chunk size", "Total", "Avg chars"],
                    datatype=["str", "number", "number", "number"],
                    interactive=False,
                    wrap=True,
                )

            mode_radio = gr.Radio(
                choices=["Chat", "Search"],
                value="Chat",
                label="Mode",
            )

        # MAIN CHAT AREA 
        with gr.Column(scale=4):
            chatbot = gr.Chatbot(
                label="",
                elem_id="chatbox",
                height=600,
                placeholder="<div style='text-align:center;padding:2rem;opacity:0.4'>Upload a PDF and ask anything about it</div>",
            )
            with gr.Row():
                msg_box = gr.Textbox(
                    placeholder="Ask a question… (or enter a search query)",
                    label="",
                    scale=5,
                    show_label=False,
                    lines=1,
                )
                send_btn = gr.Button("Send", variant="primary", scale=1)
            clear_btn = gr.Button("🗑 Clear chat", size="sm", variant="secondary")

    # Event wiring


    STAT_OUTPUTS = [stat_chunks, stat_dims, stat_csize, stat_avg,
                    status_box, comparison_table]

    pdf_input.change(
        fn=process_pdf,
        inputs=[pdf_input, chunk_radio],
        outputs=STAT_OUTPUTS,
    )

    rebuild_btn.click(
        fn=rebuild_index,
        inputs=[chunk_radio],
        outputs=STAT_OUTPUTS,
    )

    send_btn.click(
        fn=chat_respond,
        inputs=[msg_box, chatbot, mode_radio],
        outputs=[chatbot, msg_box],
    )

    msg_box.submit(
        fn=chat_respond,
        inputs=[msg_box, chatbot, mode_radio],
        outputs=[chatbot, msg_box],
    )

    clear_btn.click(fn=clear_chat, outputs=[chatbot, msg_box])

# Launch

if __name__ == "__main__":
    demo.launch(
        theme=gr.themes.Default(
            primary_hue="blue",
            font=gr.themes.GoogleFont("Inter"),
        ),
        css="""
        #header { border-bottom: 1px solid #e5e7eb; padding-bottom: 8px; margin-bottom: 8px; }
        #sidebar { padding-right: 12px; border-right: 1px solid #e5e7eb; }
        #status textarea { font-size: 12px !important; color: #1d4ed8 !important;
                           background: #eff6ff !important; border-color: #bfdbfe !important; }
        #chatbox { border-radius: 10px; }
        .stat-box textarea { font-size: 16px !important; font-weight: 600 !important;
                             text-align: center !important; padding: 4px !important; }
        .stat-box label { text-align: center; font-size: 10px; }
        """,
    )